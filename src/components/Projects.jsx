...
function getScatteredPositions(baseX, baseY, count) {
  return Array.from({ length: count }).map((_, i) => {
    const angle = Math.random() * 2 * Math.PI;
    // const radius = 250 + i * 50 + Math.random() * 60;
    const radius = 150 + i * 60 + Math.random() * 60;
...
const Projects = () => {
  ...
  useEffect(() => {
    if (activeProject) {
      ...
      const titleEl = projectRefs.current[activeProject];
      const sectionRect = sectionRef.current.getBoundingClientRect();
      const titleRect = titleEl?.getBoundingClientRect();

      const baseX = titleRect ? titleRect.left + titleRect.width / 2 - sectionRect.left : sectionRect.width / 2;
      const baseY = titleRect ? titleRect.top + titleRect.height / 2 - sectionRect.top : sectionRect.height / 2;

      const scattered = getScatteredPositions(
        baseX,
        baseY,
        2 + (hovered.images?.length || 0)
      );
    ...
    <section ref={sectionRef} className={styles.projects} onMouseLeave={handleMouseLeave}>
      ...
      <div className={styles.projectsList}>
        {projects.map((project) => (
          <motion.div
            ...
            animate={
              activeProject === project.id
                ? { scale: 1.05, color: "rgba(70, 130, 180, 0.9)" }
                : { scale: 1, color: "rgba(70, 130, 180, 0.7)" }
            }
      ...
      <AnimatePresence>
        {activeProject && (() => {
          ...
          const initialCardPosition = titleRect
            ? {
                x: titleRect.left + titleRect.width / 2 - sectionRect.left,
                y: titleRect.top + titleRect.height / 2 - sectionRect.top,
                rotate: 0,
              }
            : {
          ...
          return (
            ...
            {hovered.images && hovered.images.map((img, idx) => {
              const cardIdx = 2 + idx;
              const scatter = scatteredCards[cardIdx];
              return scatter && (
                <ScatteredImage
